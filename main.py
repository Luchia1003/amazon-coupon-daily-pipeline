...
============================================
Runs once daily:
  1. Login to Seller Central (reuse session if valid)
...
from seller_login import get_authenticated_page, close_session
...
async def run_pipeline():
    ...
    playwright, browser, context, page = await get_authenticated_page()

    try:
        # ------------------------------------------------------------------
        # Step 2: Pull all coupons
        # ------------------------------------------------------------------
        raw_coupons = await fetch_all_coupons(page)
        ...
        for summary in summaries:
            ...
            try:
                raw_detail = await fetch_coupon_detail(page, promotion_id)
            ...
            if not product_selection_id:
                logger.warning(f"[{promotion_id}] No PRODUCT_SELECTION_ID found — creating null-product row.")
            ...
            # Step 4 — Products
            try:
                products = await fetch_products(page, product_selection_id)
            except Exception as e:
            ...
            for product in products:
                ...
                # Step 5 — SKU mapping
                # Priority 1: SKU already present in product API response
                sku_from_product = product.get("SKU_FROM_PRODUCT_API")
                if sku_from_product:
                    skus = [sku_from_product]
                else:
                    # Priority 2 & 3: lookup via inventory / catalog endpoints
                    try:
                        skus = await fetch_sku_for_asin(page, asin) if asin else [None]
                    except Exception as e:
                        logger.warning(f"[{promotion_id}] SKU lookup failed for ASIN={asin}: {e}")
                        skus = [None]

                # Step 6 — One row per (PROMOTION_ID, ASIN, SKU) combination
                for sku in skus:
                    row = {
                        **summary,
                        **detail,
        ...
        await close_session(playwright, browser)
